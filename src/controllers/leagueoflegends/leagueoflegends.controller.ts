...
export class LeagueoflegendsController {
  ...
  private setImagesVersion() {
    ...
    this.ICON_PASSIVE_URL = this.ICON_PASSIVE_URL.replace(
      'VERSION',
      this.version,
    );
  ...
  private async getLatestVersion(): Promise<string> {
    const versions = await this.httpService
      .get<string[]>(`${this.BASE_URL}/api/versions.json`)
  ...
  private async getLanguages(): Promise<string[]> {
    const languages = await this.httpService
      .get<string[]>(`${this.BASE_URL}/cdn/languages.json`)
      .pipe(map(response => response.data))
      .toPromise();
    return languages;
  }
  ...
  private async getChampionData(
    ...
    try {
      const champion = await this.httpService
        .get(
          `${this.BASE_URL}/cdn/${version}/data/${lang}/champion/${name}.json`,
        )
        .pipe(map(response => response.data.data))
        .toPromise();
  ...
  private createChampion = async (champion: LeagueOfLegends) => {
    ...
    for (const spell of spells) {
      const { id, cooldownBurn, costBurn, image, rangeBurn, maxrank } = spell;
  ...
  private createLocaleChampion = async (
    champion: LeagueOfLegends,
    lang: string,
  ) => {
    const {
      //Search
    ...
    const id_champion = await this.leagueoflegendsService.findByNameChampion(
      key,
    );
    ...
    await this.leagueoflegendsService.createPassiveDescription(
      data_passive_description,
    );
